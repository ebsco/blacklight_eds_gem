...
module BlacklightEds::BlacklightEdsHelper
  ...
  def show_full_text_on_screen(result)
    HTMLEntities.new.decode(HTMLEntities.new.decode(result['FullText']['Text']['Value'])) if has_full_text_on_screen? result
  end
  ...
  def truncate_article(s, length = 250, ellipsis = '...')
    #s = strip_tags s
    #s = s.to_s.gsub(/relatesTo/, "sup")
    s = s.to_s.gsub(/(relatesTo|superscript)/, "sup")
    s = sanitize s, tags: %w(sup)
    if s.length > length
      s.to_s[0..length].gsub(/[^\w]\w+\s*$/, ellipsis).sub(/(,|;)(\...)/, ellipsis).html_safe
    else
      s.html_safe
    end
  end
